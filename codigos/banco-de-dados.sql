...
select idCliente, nome, CPF, telefone, email
from Cliente;

select idVeiculo, modelo, marca, placa, valorDiaria
from Veiculo
where estado = 'Disponível';

select v.modelo, v.marca, m.descricao, m.dataManutencao, m.custo
from Manutencao m
inner join Veiculo v on m.idVeiculo = v.idVeiculo
order by m.dataManutencao desc;

select sum(valorTotal) as TotalRecebido
from Pagamento
where estado = 'Pago';

select c.nome, l.idLocacao, p.estado, l.dataInicio, l.dataFim
from Cliente c
inner join Locacao l on c.idCliente = l.idCliente
inner join Pagamento p on l.idPagamento = p.idPagamento
where p.estado <> 'Pago';

select v.modelo, v.marca, count(lv.idVeiculo) as qtd_locacoes
from LocacaoVeiculo lv
inner join Veiculo v on lv.idVeiculo = v.idVeiculo
group by v.modelo, v.marca
order by qtd_locacoes desc;
select c.nome, v.modelo, v.marca, l.dataInicio, l.dataFim, p.valorTotal, p.estado
from Locacao l
